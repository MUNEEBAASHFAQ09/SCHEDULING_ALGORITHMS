    ...
    while(1):
        ...
        for i in range (0,count):
             ...
             for i in range(len(LIST_OF_PROCESSES)):
                    if(LIST_OF_PROCESSES[i].get_process_name()==NEW_QUEUE[0].get_process_name() and LIST_OF_PROCESSES[i].get_arrival_time()==NEW_QUEUE[0].get_arrival_time()):
                        LIST_OF_PROCESSES[i].set_waiting_time(clock_tick-(LIST_OF_PROCESSES[i].get_brust_time()+LIST_OF_PROCESSES[i].get_arrival_time()))
                        LIST_OF_PROCESSES[i].set_turnaround_time(clock_tick-LIST_OF_PROCESSES[i].get_arrival_time())
                    ...
                    temperory=PROCESSES()
                    temperory.process_name=NEW_QUEUE[0].process_name
                    temperory.arrival_time=NEW_QUEUE[0].arrival_time
                    temperory.brust_time=NEW_QUEUE[0].brust_time
                    temperory.brust_time=NEW_QUEUE[0].brust_time-quantum_time
                    NEW_QUEUE.append(temperory) 
                    NEW_QUEUE.remove(temperory)
